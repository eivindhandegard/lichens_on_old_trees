...
```{r}
  variable_summary_pine <- pine_species_richness_age |> 
select(age,bark_structure,dbh,branch_thickness,site_index,basal_area,elevation)|> 
mutate(branch_thickness = ordered(branch_thickness))
  
  summary(variable_summary_pine)
```
...
variable_summary_spruce <- spruce_species_richness_age |> 
select(age,bark_structure,dbh,branch_thickness.numeric,site_index,basal_area,elevation)
  
  summary(variable_summary_spruce)
```
...
Age: Tree age and bark structure

Size: DBH and thickness of the the thickest branch. Categorical for scots pine 0-5, 5-10, 10-20, \> 20

Productivity: Elevation, basal area and site index H40
...
The lichen epiphyte richness is correlated
...
The lichen epiphyte richness is positively related to tree age for both species, and thus the first null hypothesis is rejected. This association has the strongest evidence for spruce. We go on to the next level in the hypothesis testing
...
It is also clear from the analysis that alpha diversity is related to large branched trees for spruce trees and not necessarily just big trees
...
The full spruce model has singular random effects structure, but otherwise no error messages.
...
pinemod_size_independent  <- glmer(species~  
                      scale(age, center = T, scale = T) + slope
                    + (1|locality/plot) , family = poisson,
                data = pine_species_richness_age)
...
sprucemod_size_independent <- glmer(species~ scale(age, center = T, scale = T) + slope
                      + (1|locality) #+  (1|plot)
                      , family = poisson,
